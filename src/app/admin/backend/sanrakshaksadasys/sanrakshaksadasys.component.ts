...
@Component({
  selector: 'app-sanrakshaksadasys',
  templateUrl: './sanrakshaksadasys.component.html',
  styleUrls: ['./sanrakshaksadasys.component.css']
})
export class SanrakshaksadasysComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getSanrakshakSadasyas().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getSanrakshakSadasya(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getSanrakshakSadasya(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addSanrakshakSadasya(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateSanrakshakSadasya(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteSanrakshakSadasya(id);
    else return;
  }
