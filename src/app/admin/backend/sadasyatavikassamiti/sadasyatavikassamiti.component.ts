...
@Component({
  selector: 'app-sadasyatavikassamiti',
  templateUrl: './sadasyatavikassamiti.component.html',
  styleUrls: ['./sadasyatavikassamiti.component.css']
})
export class SadasyatavikassamitiComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getSadasyataVikasSamitis().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getSadasyataVikasSamiti(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getSadasyataVikasSamiti(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addSadasyataVikasSamiti(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateSadasyataVikasSamiti(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteSadasyataVikasSamiti(id);
    else return;
  }
