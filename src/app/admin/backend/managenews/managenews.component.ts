...
export class ManagenewsComponent implements OnInit {
  ...
  show:boolean=false;
  edit:boolean=false;
  displayedColumns: string[] = ['image','blank','headline', 'actions'];
  ...
  goShow(){
    this.firebaseService.getAllNews().subscribe(res =>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getNews(this.id).subscribe(ad=>{
  ...
  updateNews(value){
    ...
    this.nextimg='';
    this.firebaseService.updateNews(this.id, value);
    this.goShow();
  }
  ...
  goEdit(id){
    ...
    this.firebaseService.getNews(id).subscribe(res=>{
      this.exampleForm.patchValue({
  ...
  goDelete(id){
    let res = confirm("Are you sure about deleting this News?");
    if(res) this.firebaseService.deleteNews(id);
    else return;
  }
