...
@Component({
  selector: 'app-anushashansamiti',
  templateUrl: './anushashansamiti.component.html',
  styleUrls: ['./anushashansamiti.component.css']
})
export class AnushashansamitiComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getAnushashanSamitis().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getAnushashanSamiti(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getAnushashanSamiti(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addAnushashanSamiti(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateAnushashanSamiti(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteAnushashanSamiti(id);
    else return;
  }
