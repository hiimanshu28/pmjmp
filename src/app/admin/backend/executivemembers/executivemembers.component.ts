...
@Component({
  ...
  styleUrls: ['./executivemembers.component.css']
})
export class ExecutivemembersComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getExecutiveMemberss().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getExecutiveMembers(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getExecutiveMembers(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addExecutiveMembers(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateExecutiveMembers(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteExecutiveMembers(id);
    else return;
  }
