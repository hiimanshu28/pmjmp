...
@Component({
  selector: 'app-nationalbody',
  templateUrl: './nationalbody.component.html',
  styleUrls: ['./nationalbody.component.css']
})
export class NationalbodyComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getNationalBodys().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getNationalBody(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getNationalBody(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addNationalBody(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateNationalBody(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteNationalBody(id);
    else return;
  }
