...
@Component({
  selector: 'app-mediaprabhandan',
  templateUrl: './mediaprabhandan.component.html',
  styleUrls: ['./mediaprabhandan.component.css']
})
export class MediaprabhandanComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getMediaPrabhandans().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getMediaPrabhandan(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getMediaPrabhandan(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addMediaPrabhandan(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateMediaPrabhandan(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteMediaPrabhandan(id);
    else return;
  }
