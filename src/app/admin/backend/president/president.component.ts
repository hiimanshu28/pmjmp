...
export class PresidentComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getPresidents().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getPresident(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getPresident(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addPresident(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updatePresident(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deletePresident(id);
    else return;
  }
