...
export class ManageEventsComponent implements OnInit {
  ...
  goShow(){
    ...
    this.dataSource.paginator._intl.itemsPerPageLabel ='Events per page:';
    });
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getEvent(this.id).subscribe(ad=>{
  ...
  updateEvent(value){
    ...
    this.nextimg='';
    this.firebaseService.updateEvent(this.id, value);
    this.goShow();
  }
  ...
  goEdit(id){
    ...
    this.firebaseService.getEvent(id).subscribe(res=>{
      this.exampleForm.patchValue({
  ...
  goDelete(id){
    let res = confirm("Are you sure about deleting this Event?");
    if(res) this.firebaseService.deleteEvent(id);
    else return;
  }
