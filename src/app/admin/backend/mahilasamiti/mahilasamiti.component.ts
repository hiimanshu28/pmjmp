...
@Component({
  selector: 'app-mahilasamiti',
  templateUrl: './mahilasamiti.component.html',
  styleUrls: ['./mahilasamiti.component.css']
})
export class MahilasamitiComponent implements OnInit {


  create:boolean=false;
  show:boolean=false;
  edit:boolean=false;
  displayedColumns: string[] = ['image', 'name','position','contact','rank','zone','actions'];
  ...
  goShow(){
    this.firebaseService.getMahilaSamitis().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getMahilaSamiti(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getMahilaSamiti(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addMahilaSamiti(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateMahilaSamiti(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteMahilaSamiti(id);
    else return;
  }
