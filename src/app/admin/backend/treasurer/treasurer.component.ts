...
export class TreasurerComponent implements OnInit {
  ...
  goShow(){
    this.firebaseService.getTreasurers().subscribe(members =>{
  ...
  goEdit(id){
    ...
    this.firebaseService.getTreasurer(id).subscribe(member=>{
  ...
  upload(event){
    if(this.id){
      this.firebaseService.getTreasurer(this.id).subscribe(ad=>{
  ...
  addMember(value){
      this.firebaseService.addTreasurer(value);
      this.resetFields();
      this.nextimg='';
      this.goShow();
  }
  ...
  updateMember(value){
    ...
    this.nextimg='';
    this.firebaseService.updateTreasurer(this.id, value);
    this.goShow();
  }

  goDelete(id){
    let res = confirm("Are you sure about deleting this Member?");
    if(res) this.firebaseService.deleteTreasurer(id);
    else return;
  }
